...
test("returns the longest word in a mixed case string of text", () => {
  expect(longestWord("Top Shelf Web Development Training on Scotch")).toEqual(
    "Development"
  );
});

test("returns the longest word in a lowercase string", () => {
  expect(longestWord("the ultimate guide to js algorithms")).toEqual(
    "algorithms"
  );
});
test("returns the longest word in an uppercase string", () => {
